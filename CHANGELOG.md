...
- **No more storm of black console windows while OmniVoice sets itself up on Windows.** First-run bootstrap spawns dozens of helper processes — `uv`, the managed-Python interpreter, and a series of capability probes — and on Windows each one flashed its own `cmd`-style console window, so a fresh install looked like it was being swarmed by pop-up terminals. Every bootstrap subprocess now runs windowless through a single chokepoint (the same treatment the backend process already had), so nothing flashes; output still streams to the setup screen. macOS and Linux are unaffected. (#1178)
